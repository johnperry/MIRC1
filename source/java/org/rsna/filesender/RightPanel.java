...
public class RightPanel extends JPanel
				...
				if ((currentSelection != null) && !destination.trim().equals("")) {
					try {
						sender = new Sender(
							...
							currentSelection,
							subdirectories,
							footerPanel.queueElements.isSelected(),
							footerPanel.deleteQueueElements.isSelected(),
	...
	class FooterPanel extends JPanel implements ActionListener {
		...
		public JCheckBox queueElements;
		public JCheckBox deleteQueueElements;
		...
		public FooterPanel() {
			...
			String indirect = (String)properties.getProperty("queue-elements");
			if (indirect == null) {
				indirect = "yes";
				properties.setProperty("queue-elements",indirect);
			}
			queueElements = new JCheckBox("Process queue elements",indirect.equals("yes"));
			queueElements.setBackground(background);

			String deleteQE = (String)properties.getProperty("delete-queue-elements");
			if (deleteQE == null) {
				deleteQE = "no";
				properties.setProperty("delete-queue-elements",deleteQE);
			}
			deleteQueueElements = new JCheckBox("Delete transmitted queue elements",indirect.equals("yes"));
			deleteQueueElements.setBackground(background);

			button = new JButton("Send");

			Box box0 = new Box(BoxLayout.X_AXIS);
			box0.add(queueElements);
			queueElements.addActionListener(this);
			box0.add(Box.createHorizontalGlue());
			this.add(box0);

			Box box0a = new Box(BoxLayout.X_AXIS);
			box0a.add(deleteQueueElements);
			deleteQueueElements.addActionListener(this);
			box0a.add(Box.createHorizontalGlue());
			this.add(box0a);
		...
		public void actionPerformed(ActionEvent evt) {
			...
			properties.setProperty("queue-elements",(queueElements.isSelected() ? "yes" : "no"));
			deleteQueueElements.setSelected(queueElements.isSelected() && deleteQueueElements.isSelected());
			properties.setProperty("delete-queue-elements",(deleteQueueElements.isSelected() ? "yes" : "no"));
		}
	}
