...
public class MircIndexEntry implements Serializable {
	 ...
	 */
	public MircIndexEntry(File file, String path, Document doc, File xslFile) throws Exception {
		...
		root.setAttribute("filename", path);
		Object[] params = { "lmdate", Long.toString(lmdate) };
		Document qrDoc = XmlUtil.getTransformedDocument( doc, xslFile, params );
	 ...
	 */
	public boolean allows(TomcatUser tcUser) {
		if (isPublic) return true;
		if (tcUser == null) return false;
		if (tcUser.isAdmin) return true;
		if (users.contains(tcUser.username)) return true;
		String[] userroles = tcUser.roles.split(",");
	...
	private void insert( Node node, String name, HashSet set ) {
