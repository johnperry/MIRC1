...
public class ZipObject extends FileObject {
	...
	public ZipObject(File file) throws Exception {
		...
		if (!manifest.equals("")) {
			try { manifestXML = XmlUtil.getDocumentFromString(manifest); }
	...
	/**
	 * Set the standard extension for a ZipObject (".zip").
	...
	/**
	 * See if the zip file contains a manifest.
	 * @return true if the zip file contains a manifest, false otherwise.
	 */
	public boolean hasManifest() {
		return manifestXML != null;
	}
	...
	public String getDate() {
		if (manifestXML == null) return "";
		return manifestXML.getDocumentElement().getAttribute("date");
	}
	...
	public ZipEntry[] getEntries() {
		try {
			...
			entries = (ZipEntry[])list.toArray(entries);
			return entries;
		}
