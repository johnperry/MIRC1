...
IMAGESECTION.prototype.load = function() {
	...
	function getImage() {
		var pImage = null;
		while ((nextToLoad < thisObject.IMAGESETs.length) && !(pImage = thisObject.IMAGESETs[nextToLoad].pImage)) nextToLoad++;
		if (pImage) {
			nextToLoad += 1;
	...
	function getNextImage() {
		if (req.success()) {
			window.setTimeout(getImage, 10);
		}
	}
	function getOSImage() {
		var osImage = null;
		while ((nextToLoad < thisObject.IMAGESETs.length) && !(osImage = thisObject.IMAGESETs[nextToLoad].osImage)) nextToLoad++;
		if (osImage) {
			nextToLoad += 1;
	...
	function getNextOSImage() {
		if (req.success()) {
			window.setTimeout(getOSImage, 10);
		}
	}
}
