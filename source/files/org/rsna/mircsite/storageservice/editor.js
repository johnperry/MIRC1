...
function previewClicked() {
	if (!checkUnsupportedElements()) return;
	var form = document.getElementById("author-service-form");
	//If the document has not already been
	//saved, then treat this button as a save.
	var action = form.action;
	if (action.toLowerCase().indexOf("/submit") != -1) {
		saveClicked();
		return;
	}
	lastSave = getDocumentText();
...
function getTAText(textarea) {
	return filter(normalize(trim(textarea.value)));
}
